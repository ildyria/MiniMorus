...
\section{Linear Trail for \texorpdfstring{\MiniMORUS}{MiniMORUS}}
...
\begin{figure}
  ...
  \begin{tikzpicture}[xscale=0.65,yscale=1.5]%{{{
    ...
    \draw[trail, alpha]
      ...
      (xor0) -- (lll0) (lll0) -- (W40) node[below,inner sep=2pt] {$i+b_0$}
      ;
  ...
  \begin{tikzpicture}[xscale=0.65,yscale=1.5]%{{{
    ...
    \draw[trail, beta]
      (C) -- node[right] {$i$} (lll-1)
      (lll-1) -- (tlll-1) (lll-1) -- (xor-1) (xor-1) -- (xnd-1)
      (tlll-1) -- (W-20) node[above,inner sep=2.5pt] {$i$}
      (xor-1) -- (txor-1) (txor-1) -- (W-21) node[above,inner sep=2.5pt] {$i$}
      (xnd-1) -- (and-1)
      (W40) node[below,inner sep=2pt] {\phantom{$i$}}
      ;
  \end{tikzpicture}%}}}
  ...
  \begin{tikzpicture}[xscale=0.65,yscale=1.5]%{{{
    \printstate
    \draw[trail, gamma]
      (W-21) node[above,inner sep=2.5pt] {$i$} -- (xnd1) (xnd1) -- (xor1) (xor1) -- (lll1)
      (xnd1) -- (and1)
      (xor1) -- (txor1)
      (txor1) -- (W-24) node[above,inner sep=2.5pt] {$i$}
      (lll1) -- (W41) node[below,inner sep=2pt] {$i+b_1$}
      ;
  \end{tikzpicture}%}}}
  ...
  \begin{tikzpicture}[xscale=0.65,yscale=1.5]%{{{
    \printstate
    \draw[trail, delta]
      (W-24) node[above,inner sep=2.5pt] {$i$} -- (xnd4) (xnd4) -- (xor4) (xnd4) -- (and4)
      (xor4) -- (lll4) (xor4) -- (txor4) (txor4) -- (W42) node[below,inner sep=2pt] {$i$}
      (lll4) -- (W44) node[below,inner sep=2pt] {$i+b_4$}
      ;
  \end{tikzpicture}%}}}
  ...
  \begin{tikzpicture}[xscale=0.65,yscale=1.5]%{{{
    \printstate
    \draw[trail, epsil]
      (W-22) node[above,inner sep=2.5pt] {$i$} -- (xnd2) (xnd2) -- (xor2) (xnd2) -- (and2)
      (xor2) -- (lll2) (xor2) -- (txor2) (txor2) -- (W40) node[below,inner sep=2pt] {$i$}
      (lll2) -- (W42) node[below,inner sep=2pt] {$i+b_2$}
      ;
  \end{tikzpicture}%}}}
...
\begin{figure}
  ...
  \begin{tikzpicture}[xscale=3, yscale=1.5, every node/.style={font=\scriptsize,inner sep=2pt}]
    ...
    \draw[dashed] (\S{4.125}{-1.5}) -- ++(\S{0}{7}) -- ++(\S{1}{-1}) -- ++(\S{0}{7});
    ...
    \begin{scope}[xshift=2.25cm,yshift=-6.5cm]
      \foreach \r in {1,...,4} { \draw[gray, rounded corners=2pt] (0,-\r*1.5) rectangle ++(1.5,-1.5); }
      \foreach \r in {0,...,4} { \draw (\S{1}{\r}) node[above] {$S_{\r}$}; }
      \draw (\C{0}|-\S{1}{0}) node[above] {$C$};
  ...
  \caption{\MiniMORUS: two approximations for $S_{2,0}^2$. Numbers in each diagram denote bit positions used in the linear approximation, i.e.~subscripts of $\alpha,\beta,\gamma,\delta$ and $\varepsilon$. $\chi_1$ and $\chi_2$ are two halves of the full trail which we experimentally verify.}
...
\begin{alignat*}{2}
\mathbf{A}^t_i:\quad&& C^t_i \oplus S^{t+1}_{0,i+b_0} &= S^t_{1,i} \oplus S^t_{3,i} \oplus S^t_{1,i} \cdot S^t_{2,i} \oplus S^t_{2,i} \cdot S^t_{3,i}\\
\mathbf{B}^t_i:\quad&& C^t_i \oplus S^t_{0,i} \oplus S^t_{1,i} &= S^t_{2,i} \cdot S^t_{3,i} \\
\mathbf{C}^t_i:\quad&& S^t_{1,i} \oplus S^{t+1}_{1,i+b_1} \oplus S^t_{4,i} &= S^t_{2,i} \cdot S^t_{3,i}\\
\mathbf{D}^t_i:\quad&& S^t_{4,i} \oplus S^{t+1}_{4,i+b_4} \oplus S^{t+1}_{2,i} &= S^{t+1}_{0,i} \cdot S^{t+1}_{1,i}\\
\mathbf{E}^t_i:\quad&& S^t_{2,i} \oplus S^{t+1}_{2,i+b_2} \oplus S^{t+1}_{0,i} &= S^t_{3,i} \cdot S^t_{4,i}
\end{alignat*}
...
The equation for \MiniMORUS[640] is very similar, and is given in the full version of this paper \cite{fullversion}.
...
%That the correlation is so high may be surprising: indeed, the full trail uses trail fragment $\varepsilon^t_i$ 1 time, $\delta^t_i$ 2 times, $\gamma^t_i$ 4 times, $\beta^t_i$ 8 times, and $\alpha^t_i$ 9 times. 
The correlation is surprising high. The full trail uses trail fragments $\varepsilon^t_i$, $\delta^t_i$, $\gamma^t_i$, $\beta^t_i$, and $\alpha^t_i$, once, twice, 4 times, 8 times, and 9 times, respectively.
Since each trail fragment has a weight of 1, this would suggest that the total weight should be $1+2+4+8+9 = 24$ rather than 16. However, when combining trail fragments $\beta_i$ and $\gamma_i$, notice that the same AND is computed at the same step between registers $S_2$ and $S_3$ (equivalently, notice that the right-hand side of equations $\mathbf{B}^t_i$ and $\mathbf{C}^t_i$ is equal). In both cases it is approximated by zero. When XORing the corresponding equations, these two ANDs cancel each other, which saves two AND gates. Since $\gamma^t_i$ is used four times in the course of the full trail, this results in saving 8 AND gates overall, which explains why the final correlation is $2^{-16}$ rather than $2^{-24}$.

\subsection{Experimental Verification}

To confirm that our analysis is correct, we ran experiments on an implementation of \MiniMORUS[1280] and \MiniMORUS[640].
We consider two halves $\chi_1$ and $\chi_2$ of the full trail (depicted on \Cref{fig:twoapproximations}), as well as the full trail itself, denoted by $\chi$. In each case, we give the weight predicted by the analysis from the previous section, and the weight measured by our experiments.
%In both cases, we measured the correlation of two halves $\chi_1$ and $\chi_2$ of the full trail, as well as the correlation of the full trail itself (denoted by $\chi$).
Results are displayed on \Cref{tab:miniapproximations}.
While our analysis predicts a correlation of $2^{-16}$, experiments indicate a slightly better empirical correlation of $2^{-15.5}$ for \MORUS[640]. The discrepancy of $2^{-0.5}$ probably arises from the fact that register bits across different steps are not completely independent.
