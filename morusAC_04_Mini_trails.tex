...
While our analysis from the previous section predicts a correlation of $2^{-16}$, experiments indicate a slightly better empirical correlation of $2^{-15.5}$ for \MiniMORUS[640]. The discrepancy of $2^{-0.5}$ probably arises from the fact that register bits across different steps are not completely independent.

In order to preserve anonymity, our implementation is attached to the submission as auxiliary material. In the full version of this paper, a link will be provided.
