...
\begin{abstract}
  ...
  a 128-bit or 256-bit key. For all versions the security
  ...
  As our main result, we exhibit a
  linear bias in the keystream of full \MORUS, which can be used to distinguish its output
  ...
  would be expected for a 256-bit secure cipher.  For
  \MORUS[640], the same attack results in a bias of $2^{-73}$, which does not threaten
...
%
  To identify this bias, we make use of rotational symmetries in \MORUS
  ...
  \MORUS called \MiniMORUS, which aid in the
...
%times, regardless of whether each encryption uses a different key.
  ...
  nonce-misue setting when initialization is reduced from 16 to 10
