...
To simplify the description of the attack, we assume all plaintext blocks are zero. However the same results would hold as long as the plaintext is fixed. Let $w$ denote the word size, i.e. $w = 32$ in the case of \cipher{MORUS640}, and $w = 64$ for \cipher{MORUS1280}. Recall that the inner state of the cipher consists of five $4w$-bit registers $S_0,\dots,S_4$, each containing four $w$-bit words.
...
The trail fragments are depicted on \Cref{fig:minimorusfragments}, and may be represented as follows (recall that the $b_i$'s are rotation constants):
\begin{alignat*}{2}
...
\]
As a result, the trail fragment $\alpha^t_i$ also has a weight of 1. Another way to look at this phenomenon is that the trail holds for two different approximations of the AND gates: the alternative approximation is depicted by a dashed line on \Cref{fig:minimorusfragments}.
...
The trail for \cipher{MORUS1280} is very similar, and is given in \Cref{sec:traileq}.

\subsection{Bias of the Trail}
\label{sec:minibias}

In the equation for \cipher{MORUS640} from the previous section, each line on the right-hand side of the equality involves distinct terms, and each line has a weight of 1. By the piling-up lemma, it follows that if we assume distinct $S^t_{i,j}$ terms to be uniform and independent, then the expression on the right-hand side has a weight of 16. Hence the linear combination of ciphertext bits on the left-hand side has a bias of $2^{-16}$. The same holds for \cipher{MORUS1280} (cf. \Cref{sec:traileq}).
...
In the previous section, we have exhibited a linear trail on the reduced ciphers \cipher{MiniMORUS640} and \cipher{MiniMORUS1280}. We now turn to the full ciphers \cipher{MORUS640} and \cipher{MORUS1280}. As explained in \Cref{sec:introminimorus}, translating a trail from one of the \cipher{MiniMORUS} ciphers to the corresponding full \cipher{MORUS} cipher just amounts to making the linear combinations symmetric---indeed, that was the point of introducing \cipher{MiniMORUS} in the first place. That is, for every $S^t_{i,j}$ (resp. $C^t_j$) term in every trail fragment and every equation, we \emph{symmetrize} the term by adding in the terms $S^t_{i,j+w}$, $S^t_{i,j+2w}$, $S^t_{i,j+3w}$ (resp. $C^t_{j+w}$, $C^t_{j+2w}$, $C^t_{j+3w}$), where as usual $w$ denotes the word size. For example, if $w=32$ (for \cipher{MORUS640}), the term $S^3_{2,0}$ is symmetrized into:
\[
...
\oplus\; &C^2_{27} \oplus C^2_{31} \oplus C^3_{6} \oplus C^3_{12} \oplus C^3_{14} \oplus C^3_{20} \oplus C^4_{19}
\end{align*}
yields a biased linear combination on the full \cipher{MORUS640}, namely:
\begin{align*}
...
\end{align*}
We refer the reader to \Cref{sec:traileq} for the corresponding equation on \cipher{MORUS1280}.

\subsection{Bias of the Full Trail}

Due to the observations from \autoref{sec:introminimorus}, the symmetrized trail on full \cipher{MORUS} more or less consists of four copies of the original trail on \cipher{MiniMORUS}. As such, one may expect that the weight of the symmetrized trail should simply be four times the weight of the corresponding \cipher{MiniMORUS} trail, namely 64 for both \cipher{MORUS640} and \cipher{MORUS1280}. However, reality is a little more complex, as the symmetrized trail does not simply behave as four copies of the original trail.
...
\end{align}
It may be observed that the products occuring in the equation above involve 8 terms in a circular ring. The weight of this expression can be computed by brute force, and is equal to $3$.
...
In the end, for \cipher{MORUS1280}, since the trail fragment $\gamma^t_i$ is used four times, the phenomenon we just described adds a contribution of $4 \cdot 3 = 12$ to the overall weight of the full trail. This results in a total weight of $4 \cdot 16 + 12 = 76$ (where 16 is the weight of the trail on \cipher{MiniMORUS1280}). Due to the complexity of the trail, we have confirmed that this bias is correct by explicitely computing the full trail equation in \Cref{sec:traileq}, and computing its exact weight like we did for \cipher{MiniMORUS} in \Cref{sec:minibias}. That is, since the equation is quadratic, we may view it as a graph, which we split into connected components; we then compute the bias of each connected component separately by brute force, and then add up the weights of all components per the piling-up lemma. In the end, the full trail equation does confirm a weight of 76 for the full trail on \cipher{MORUS1280}.

In the case of \cipher{MORUS640}, collisions between rotation constants further complicate the analysis. More specifically, when using trail fragment $\beta^t_i$, the term $S^t_{2,i} \cdot S^t{3,i}$ occurs. As explained previously, a quasi-collision with the term $S^t_{2,i} \cdot S^t{3,i-w}$ from trail fragment $\gamma^t_i$ results in \Cref{eq:8circle}. However for \cipher{MORUS640}, trail fragment $\alpha^t_{i+d}$ is once used with an offest of $d = b_1+b_4-b_0-b_2 = 31+13-5-7 = 0 \;\text{mod}\; 32$ relative to $\gamma^t_i$. This creates another term $S^t_{2,i} \cdot S^t{3,i}$, and destroys one of the four occurences of \Cref{eq:8circle}. In the end, when computing the full trail equation on \cipher{MORUS640}, we get that the weight of the trail is 73 (cf. \Cref{sec:traileq}).
...
The design document of \cipher{MORUS} imposes a limit of $2^{64}$ encrypted blocks for a given key. However, since our attack is independent of the encryption key, and hence immune to rekeying, the limitation does not apply: all that matters for our attack is that the same plaintext be encrypted enough times.
...
However, the existence of this trail does hint at some weakness in the design of \cipher{MORUS}. Indeed, a notable feature of the trail is that the values of rotation constants are mostly irrelevant: a similar trail would exist for most choices of the constants. That it is possible to build a trail that ignores rotation constants may be seen as surprising. This would have been prevented by adding a word-wise rotation to one of the state registers at the input of the ciphertext equation---it seems that this would defeat any trail of the type uncovered in this work.
...
In this section, we provide full trail equations for all variants of \cipher{MORUS}. In each case, we decompose the right-hand side of the equality (involving state bits) into connected components, and compute the weight of each connected connected component. By the piling-up lemma, if we assume that the state bits are uniformly random and independent, then the weight of the full equation is equal to the sum of the weights of the connected components.
