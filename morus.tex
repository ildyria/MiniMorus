...
\definecolor{epsilon}{HTML}{CB4B16}
...
\begin{figure}
  ...
  \begin{tikzpicture}[xscale=0.75,yscale=1.5]%{{{
    ...
    \draw[trail, alpha]
      ...
      ;
  \end{tikzpicture}%}}}
  \caption*{$\alpha_i$: weight 1} %$C^j_i, S^{j-1,0}_{i+5}$ ($w\!=\!1$)}
  \end{subfigure}
  \hfill
  \begin{subfigure}{.32\textwidth}
  \centering
  \begin{tikzpicture}[xscale=0.75,yscale=1.5]%{{{
    ...
    \draw[trail, beta]
      ...
      ;
  \end{tikzpicture}%}}}
  \caption*{$\beta_i$: weight 0} %$C^j_i, S^{j,0}_i, S^{j,1}_i$ ($w\!=\!0$)}
  \end{subfigure}
  \hfill
  \begin{subfigure}{.32\textwidth}
  \begin{tikzpicture}[xscale=0.75,yscale=1.5]%{{{
...
\begin{figure}
  ...
  \begin{subfigure}{\textwidth}
    ...
    Approximation 1 for $S^{2,2}_0$ (weight 7):

    Step 0: $\alpha_{27}$ (weight 1)

    Step 1: $\alpha_{8,26}, \beta_0, \gamma_0, \delta_0$ (weight 4)

    Step 2: $\alpha_7, \beta_{13,31}, \gamma_{13}$ (weight 2)

    Step 3: $\beta_{12}$ (weight 0)

    Step 4: ---
    \bigskip

    Approximation 2 for $S^{2,2}_0$ (weight 9):

    Step 0: ---

    Step 1: $\alpha_2$ (weight 1)

    St.2: $\alpha_{1,15,27}, \beta_7, \gamma_7, \delta_7, \varepsilon_0$ (weight 6)

    Step 3: $\alpha_{14}, \beta_{6, 20}, \gamma_{20}$ (weight 2)

    Step 4: $\beta_{19}$ (weight 0)
    \caption{\cipher{MiniMORUS-640}}
  \end{subfigure}
  \bigskip

  \begin{subfigure}{\textwidth}

    \caption{\cipher{MiniMORUS-1280}}
  \end{subfigure}
  \caption{\cipher{MiniMORUS} approximations for $S^{2,2}_0$}
\end{figure}
