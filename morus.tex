  ...
  \begin{subfigure}{\textwidth}
    \begin{tikzpicture}[xscale=3, yscale=1.5, every node/.style={font=\scriptsize,inner sep=2pt}]
      ...
      \draw[delta] (\S{1}{4}) node[above] {\,0} -- (\S{2}{4}) node[above right] {13}
                   (\S{1}{4}) ++(0,-1.0) -| ([xshift=2pt]\S{2}{2}) node[below] {0} node[black] {$\times$}
                   (\M{1}{3}) node[right] {$\delta_0$};

      \draw[beta]  (\C{2}) node[above left] {31} -| ([xshift=-1pt]\S{2}{0}) node[below left] {31\!}
                   (\C{2}-|[xshift=-1pt]\S{2}{0}) -| ([xshift=0pt]\S{2}{1}) node[below left] {31\!\!}
      ...
      \begin{scope}[xshift=2.5cm]
        ...
        \draw[alpha] (\M{1}{1}) node[right] {$\alpha_2$};

        \draw[beta]  (\M{2}{0}) node[right] {$\beta_7$};
        \draw[alpha] (\M{2}{1}) node[right] {$\alpha_{1,15,27}$};
        \draw[gamma] (\M{2}{2}) node[right] {$\gamma_7$};
        \draw[delta] (\M{2}{3}) node[right] {$\delta_7$};
        \draw[epsil] (\M{2}{4}) node[right] {$\varepsilon_0$};

        \draw[beta]  (\M{3}{0}) node[right] {$\beta_{6,20}$};
        \draw[alpha] (\M{3}{1}) node[right] {$\alpha_{14}$};
        \draw[gamma] (\M{3}{2}) node[right] {$\gamma_{20}$};

        \draw[beta]  (\M{4}{0}) node[right] {$\beta_{19}$};
