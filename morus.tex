...
Basic idea using the fragments in \autoref{fig:minimorusfragments} and a fixed message $M=0$:
\begin{itemize}
    \item $\alpha_i$ approximates (one bit of) state word $S^0$ with the ciphertext
    \item $\beta_i$ approximates $S^1$ using $S^0$ and the ciphertext
    \item $\gamma_i$ approximates $S^4$ using two approximations of $S^1$ in consecutive steps
    \item $\delta_i$ approximates $S^2$ using two approximations of $S^4$ in consecutive steps
    \item $\varepsilon_i$ approximates $S^0$ using two approximations of $S^2$ in consecutive steps
\end{itemize}
Thus, we obtain a ciphertext-only approximation by appropriately connecting
$1 \times \alpha$ with $1 \times \varepsilon$, $2 \times \delta$, $4 \times \gamma$, $8 \times \beta$, $8 \times \alpha$.
The attack is essentially independent of the rotation constants $b_i$.
The resulting biased ciphertext expression is listed for both
\cipher{MiniMORUS-640} and \cipher{MiniMORUS-1280} in \autoref{tab:miniapproximations}.
Since each fragment has weight 1, we would expect a total weight of 24;
however, the approximations are not independent (\autoref{fig:todo}).
The actual weight is 16, and experiments confirm an even higher correlation of $2^{-15.5}$.
